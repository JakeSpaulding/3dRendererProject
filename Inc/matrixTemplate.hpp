...
template<unsigned int M> // size of matrix - Changed to unsigned int
struct Mbase {
    float data[M * M];
    // default constructor
    Mbase() {
...
template<unsigned int M>  // Changed to unsigned int
Mbase<M> operator*(float n, Mbase<M> const& m) {
    return(m * n);
}

// add printability
template<unsigned int M> std::ostream& operator<<(std::ostream& os, Mbase<M> const& m) {  // Changed to unsigned int
    os << "\n";
    for (unsigned int j = 0; j < M; j++) {  // Changed to unsigned int
        os << "[";
        for (unsigned int i = 0; i < M; ++i) {  // Changed to unsigned int
