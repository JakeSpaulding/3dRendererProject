...
struct fbo {
	...
	void drawMeshThreaded(Mesh const& mesh, Camera const& cam, vector<pointLight> const& lights, uint32_t tileSize = 256);  // Changed to unsigned int

	// allows the passing of multiple meshes
	void drawMeshesThreaded(vector<Mesh> const& meshes, Camera const& cam, vector<pointLight> const& lights, uint32_t tileSize = 256);  // Changed to unsigned int
